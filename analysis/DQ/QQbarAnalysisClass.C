...
void QQbarAnalysisClass::DQChecks(int n_entries=-1, float Kvcut=35, TString output="test")
{
 ...
 if(PreSelection(0,Kvcut)==false) continue;
	...
	if(mc_stable_E[i]>0 && mc_stable_E[i]<250) {
	  ...
	  if(mc_stable_isisr[i]==1) {
            h_stable_isr_phi[ihisto]->Fill(anglesmom.at(0));
            h_stable_isr_theta[ihisto]->Fill(anglesmom.at(1));
            h_stable_isr_mom[ihisto]->Fill(anglesmom.at(2));
            nstableisr++;
          }
    ...
    h_stable_isr_n[ihisto]->Fill(nstableisr);

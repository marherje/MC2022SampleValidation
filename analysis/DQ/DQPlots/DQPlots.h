...
void Compare_3Histos(TString name, std::vector<TH1F *> histos1, std::vector<TH1F *> histos2, int nfiles=2, bool normalized =true, bool log=true) {
  ...
  for(int kl=0; kl<3; kl++) {
	...
	if(kl==0) histos2.at(kl+i*3)->GetXaxis()->SetTitle("Nvtx (per jet)");
        if(kl==1) histos2.at(kl+i*3)->GetXaxis()->SetTitle("Ntracks (per sec. vtx)");
        if(kl==2) histos2.at(kl+i*3)->GetXaxis()->SetTitle("NPFOS (per jet)");*/
      }
      /*
      Int_t TotalBins=histos2.at(kl+i*3)->GetNbinsX();
      for(int bin=1;bin<TotalBins;bin++){
	Int_t bincontent=histos2.at(kl+i*3)->GetBinContent(bin);
	Float_t binerror=pow(bincontent,0.5);
	histos2.at(kl+i*3)->SetBinError(bin,binerror);
...
void Compare_3Histos(TString name, std::vector<TH1F *> histos1, bool log=true, std::vector<TString> leg={},bool norm=true, TString x="N", TString y="entries") {
  ...
  for(int i=0; i<histos1.size(); i++) {
    ...
    if(norm==true) histos1.at(i)-> DrawNormalized("histosame");
    else histos1.at(i)-> Draw("histosame");
...
void Compare_3Histos2Stack(TString name, std::vector<TH1F *> histos1, std::vector<TString> leg, bool normalize=true, TString x="momentum [GeV]", TString y="entries") {

  if(normalize==true){
    Float_t totalPFOs=histos1.at(0)->GetEntries();
    histos1.at(0)->Scale(1/totalPFOs);
    histos1.at(1)->Scale(1/totalPFOs);
    histos1.at(2)->Scale(1/totalPFOs);
  }
